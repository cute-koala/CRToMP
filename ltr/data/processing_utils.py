...
def sample_target_adaptive(im, target_bb, search_area_factor, output_sz, mode: str = 'replicate',
    ...
    # Crop image 计算在原图像上的裁减大小（以bbox中心为中心，裁减出搜索区域）
    # 相当于将bbox变成方形然后乘上倍数
    crop_sz_x, crop_sz_y = (output_sz * (target_bb[2:].prod() / output_sz.prod()).sqrt() * search_area_factor).ceil().long().tolist()

    # Get new sample size if forced inside the image 将bbox沿着某一个轴放缩到相同大小
    ...
    x1 = round(bbx + 0.5 * bbw - crop_sz_x * 0.5) # 计算放缩后的crop的左上角坐标，中心点相同
    x2 = x1 + crop_sz_x

    y1 = round(bby + 0.5 * bbh - crop_sz_y * 0.5)
    y2 = y1 + crop_sz_y

    # Move box inside image 判断放缩后的crop是否超出边界
...
def transform_box_to_crop(box: torch.Tensor, crop_box: torch.Tensor, crop_sz: torch.Tensor) -> torch.Tensor:
    ...
    box_out[2:] *= scale_factor
    # modicate bug
    box_out[:2] = box_out[:2].clip(min=0)
    box_out[2:] = box_out[2:].clip(max=crop_sz-1 - box_out[:2])
    return box_out
...
def target_image_crop(frames, box_extract, box_gt, search_area_factor, output_sz, mode: str = 'replicate',
    ...
    if masks is None:
        frame_crops_boxes = [sample_target_adaptive(f, a, search_area_factor, output_sz, mode, max_scale_change) for f, a in zip(frames, box_extract)]

        frames_crop, crop_boxes = zip(*frame_crops_boxes) # 裁减区域，裁减区域在原图上的坐标
    else:
        frame_crops_boxes_masks = [sample_target_adaptive(f, a, search_area_factor, output_sz, mode, max_scale_change, mask=m) for f, a, m in zip(frames, box_extract, masks)]
