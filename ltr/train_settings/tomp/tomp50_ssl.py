...
from ltr.dataset import Lasot, Got10k, TrackingNet, MSCOCOSeq,ImagenetVID,YouTubeVOS
...
def run(settings):
    ...
    settings.batch_size =20 #12 #26
    settings.num_workers =16 #12 #20
    ...
    settings.max_gap = 80 # 200
    settings.train_samples_per_epoch = 20000 # 40000
    settings.val_samples_per_epoch = 10000 # 10000
    settings.val_epoch_interval = 5
    settings.num_epochs =300 #300
    ...
    trackingnet_train = TrackingNet(settings.env.trackingnet_dir, set_ids=[0,1,2,3,4,5,6,8])
    coco_train = MSCOCOSeq(settings.env.coco_dir,version='2017')
    # imagenet_vid = ImagenetVID(settings.env.imagenet_dir)
    # youtube_train = YouTubeVOS(settings.env.youtubevos_dir)
    ...
    dataset_train = sampler.DiMPSampler_continue([lasot_train,trackingnet_train,got10k_train,coco_train], [1,1,1,1],
                                        # object_datasets= coco_train,
    ...
    # dataset_train = sampler.SSLSampler([youtube_train], [1],
    #                                     object_datasets= [coco_train,object_bank],
    #                                     samples_per_epoch=settings.train_samples_per_epoch, max_gap=settings.max_gap,
    #                                     num_test_frames=settings.num_test_frames,
    #                                     num_train_frames=settings.num_train_frames,
    #                                     processing=data_processing_train,
    #                                     domain=None if settings.domain_train else 'source')
    ...
    dataset_val = sampler.DiMPSampler([got10k_val], [1], samples_per_epoch=settings.val_samples_per_epoch,
                                      max_gap=settings.max_gap, num_test_frames=1,
                                      num_train_frames=2, processing=data_processing_val)

    loader_val = LTRLoader('val', dataset_val, training=False, batch_size=settings.batch_size, num_workers=settings.num_workers,
                           shuffle=False, drop_last=True, epoch_interval=settings.val_epoch_interval, stack_dim=1,pin_memory=True)
    ...
    objective = {'giou': GIoULoss(), 'test_clf': ltr_losses.LBHinge(threshold=settings.hinge_threshold),
                 'domain_classify':nn.CrossEntropyLoss(label_smoothing=0.1),'iou':nn.MSELoss()}

    loss_weight = {'giou': settings.weight_giou, 'test_clf': settings.weight_clf,'domain_clf':1}

    actor = actors.ToMPActor_continue(net=net,objective=objective, loss_weight=loss_weight,process=data_processing_train)

    # Optimizer
    optimizer = optim.AdamW([
        {'params': actor.net.head.parameters(), 'lr': 1e-4},  # 1e-4
        {'params': actor.net.feature_extractor.layer3.parameters(), 'lr': 2e-5},  # 2e-5
    ], lr=2e-4, weight_decay=0.0001)

    lr_scheduler = optim.lr_scheduler.MultiStepLR(optimizer, milestones=[60, 80, 90], gamma=0.2)
    ...
    trainer = LTRTrainer(actor, [loader_train, loader_val], optimizer, settings, lr_scheduler,
                         freeze_backbone_bn_layers=settings.freeze_backbone_bn_layers,autocast=settings.autocast)
