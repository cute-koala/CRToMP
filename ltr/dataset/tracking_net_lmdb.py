...
import json
from ltr.data.lmdb_utils import decode_img, decode_str
...
class TrackingNet_lmdb(BaseVideoDataset):
    ...
    def __init__(self, root=None, image_loader=jpeg4py_loader, set_ids=None, data_fraction=None):
        ...
        # self.sequence_list = list_sequences('/media/dy/ext4/yuandi/TrackingNet', set_ids)
    ...
    def _read_bb_anno(self, seq_id):
        ...
        gt_str_list = decode_str(self.root,os.path.join("TRAIN_%d" % set_id,"anno", vid_name + ".txt")).split('\n')[:-1]
    ...
    def _get_frame(self, seq_id, frame_id):
        set_id = self.sequence_list[seq_id][0]
        vid_name = self.sequence_list[seq_id][1]
        return decode_img(self.root,os.path.join("TRAIN_%d" % set_id,"frames", vid_name, str(frame_id) + ".jpg"))
